...
| Bulk backend foundation              |      76% | Framework-neutral helpers now cover import, injected MIME sniffing, settings lookup, queueing, full queue draining, active/incomplete requeueing, derived session counters, per-job size state, selected export plans, export summaries, stale export guards, selection context, command action state, processing, and URLs.        |
