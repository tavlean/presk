...
| Area                                 | Progress | State                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------ | -------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Stabilization, CI, and security      |      92% | Audit is clean, CI covers Ubuntu, Windows, and macOS, WebP output plus saved WebP side settings have automated local production-build smoke coverage, extensionless PNG input is covered in browser smoke, saved-settings storage is failure-tolerant, side output reset behavior is covered, compressed output filenames are safer, and built-in decode releases ImageBitmap resources.                                              |
...
| Bulk backend foundation              |      80% | Framework-neutral helpers now cover import, injected MIME sniffing, settings lookup, queueing, full queue draining, active/incomplete requeueing, derived session counters, drift-tolerant queue scheduling and transitions, metadata-only session snapshots with validated parsing, per-job size state, selected export plans, export summaries, stale export guards, selection context, command action state, processing, and URLs. |
