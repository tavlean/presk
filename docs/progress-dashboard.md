...
| Repo simplification                  |      76% | Dependency/script cleanup, Node 24 baseline alignment, storage centralization, storage failure handling, result-cache coverage, shared abort handling, shared/service-worker type cleanup, static-build utility cleanup, tested single-image work planning/work-start scheduling/active job bookkeeping/work abort planning/side job execution planning/side encode planning/side result assembly/job derivation/display/render-state/options-render-state/panel-layout/output-draw-state/output-control-state/output-preview/source/preprocessor/saved-settings/saved-settings-availability/saved-settings-actions and feedback/side-copy feedback/processing-error messages/encoder-select state/processor-control state/resize-preset state/result-size state/result-loading state/result-download state/initial-side-state/side-state/editor-update/encoder-support helpers, removal of deprecated `componentWillReceiveProps` usage, and removal of maintained app/lib TS suppressions have reduced some legacy loose typing, scattered browser APIs, and old Preact assumptions. Codec surface, Preact coupling, and custom Rollup complexity still remain. |
| Svelte/SvelteKit migration readiness |      61% | Shared pipeline, single-image work planning/work-start scheduling/active job bookkeeping/work abort planning/side job execution planning/side encode decisions/side result assembly/job derivation/source defaults/preprocessor changes/display render selectors/composed render-state/options render state/panel layout state/output draw state/output control state/output preview state/editor update effects/encoder support/encoder-select state/processor-control state/resize-preset state/result-size state/result-loading state/result-download state/saved-settings availability/saved-settings-actions and feedback/side-copy feedback/processing-error messages/initial-side-state/side-state helpers, removal of deprecated `componentWillReceiveProps` usage, and bulk logic extraction have started, and the current Svelte 5/SvelteKit migration guidance is documented. UI migration should wait until behavior is better tested.                                                                                                                                                                                                                |
