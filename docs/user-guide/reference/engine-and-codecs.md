...
| JPEG XL enc/dec        | libjxl                           | commit `9f544641ec83f6abd9da598bdd08178ee8a003e0`                   | yes (`jxl_enc_mt`)    | yes (`jxl_enc_mt_simd`) | enc+dec, menu "JPEG XL", `image/jxl`; SvelteKit uses single-thread encode                                      |
| MozJPEG enc            | mozilla/mozjpeg                  | `v3.3.1` (built `--with-build-date=squoosh`)                        | no                    | no                      | enc, menu "JPEG" (encoder in tooltip), `image/jpeg`                                                            |
| OxiPNG enc             | oxipng (crates.io)               | `9.0` (normal + parallel wasm-pack)                                 | parallel build exists | no                      | enc, menu "PNG" (encoder in tooltip), `image/png`; SvelteKit uses single-thread encode                         |
