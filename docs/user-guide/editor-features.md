...
The image area is a `<two-up>` custom element containing two synced `<pinch-zoom>` panes; only the left pane is actually driven, and pointer/wheel events are cloned and re-dispatched to it so the right pane can mirror its transform — that's what keeps the two halves locked together. When a side is resized with the "Contain" fit method, its canvas is pinned to the original source dimensions with CSS `object-fit: contain`, so a smaller output stays letterboxed inside the original footprint and the split line still lines up across both halves.
