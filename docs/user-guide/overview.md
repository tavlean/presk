...
> Sqush is a free, in-browser image compressor: load a picture, compare the original against a compressed version side by side, and watch quality and file size change live — all on your own device, nothing uploaded.

## What Sqush is and when to use it

Sqush makes image files smaller. You open an image, pick an output format and some settings, and Sqush re-encodes the picture so it takes up fewer bytes — handy for faster-loading websites, smaller email attachments, or fitting more photos in less space. The trick to good compression is balancing **file size** against **visual quality**, and Sqush is built to let you see that trade-off directly: it shows your original picture next to the compressed result so you can decide how far to push it.
...
> **Jargon check — "compress" / "encode":** _Compressing_ an image means re-saving it in a way that uses fewer bytes. _Encoding_ is the technical word for that re-saving step (turning raw pixels into a file in a given format like JPEG or PNG). In Sqush the two mean the same thing.

## The mental model: before vs. after, side by side

The heart of Sqush is a **two-up compare view** — one image, shown two ways at once:
...
When you change a setting, Sqush rebuilds that side's preview through three steps:

1. **Decode** — your file is read and turned into raw pixels (an in-memory image). This is what lets Sqush re-save it in any format.
...
> **Jargon check — "codec":** short for _coder/decoder_ — the engine that knows how to write (and read) a particular image format. Sqush bundles one codec per format (for example MozJPEG for JPEG, OxiPNG for PNG).
...
1. **Load an image.** Drag and drop a file onto the window, or click to open your computer's file picker. Sqush reads common formats — JPEG, PNG, WebP, GIF, BMP, and (where your browser supports them) AVIF and JPEG XL; SVG files are rasterized (converted to pixels).
2. **Look at the compare view.** The left pane shows your original. The right pane immediately shows a first compressed preview (Sqush starts you on **WebP** by default, while the left side stays on the original).
...
Sqush is **local-first**: the entire pipeline — decode, preprocess, encode — runs inside your browser. No uploads, no servers, no telemetry, and no pixels ever leave your device. The compression engines are WebAssembly modules served from Sqush's own files, not fetched from anywhere else.

Because of this design, Sqush also works **offline**: after the first visit, a service worker caches the app so you can use it with no internet connection at all.

## Tips & pitfalls

- **The left side is your reference.** Keep it on the original so the slider comparison is meaningful. (Sqush lets each side use any format, including an "Original (no re-encode)" passthrough that keeps the source bytes unchanged.)
