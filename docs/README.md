...
| [lab-intro-page.md](lab-intro-page.md) | The 2026-07-10 intro-page lab: four dev-only landing variants behind `/lab/intro` (billboard / frame / split / ledger — minimal full-viewport drop section, light+dark, real drop handling, Nucleo icon set), shared mechanics, and promotion notes. Decision PENDING. | Evaluating/continuing the landing re-design; promoting a variant to production. | The maintainer picks a variant, a variant is promoted/deleted, or the intro design changes. |
