...
| 3 | **Bulk optimization** | [bulk-ui-design-options.md](bulk-ui-design-options.md) | 🟡 Lab live | **Top product priority (maintainer, 2026-07-02).** Engine already built + proven headless; layout lab BUILT at `/lab/bulk` (dev-only) and now consolidated into one grid/focus UI with normalized no-wasted-work reprocessing. Phase 0 engine unit tests landed: Vitest + 63 bulk-engine/helper cases ([test-plan.md](test-plan.md) §4). Multi-Format Compare follows bulk ([road-map.md](road-map.md)). |
