...
| [lab-editor-restyle.md](lab-editor-restyle.md) | The 2026-07-07 editor re-style lab: two dev-only experiments (`/lab/porcelain` light-airy-squircle, `/lab/darkroom` rail+inspector+filmstrip), the reference→feature mappings, the `light-dark()` theming mechanism, harvested feature ideas, and the pending pick-a-direction decision. | Evaluating/continuing the editor re-style; adding app-wide theming; designing the bulk filmstrip. | The maintainer picks a direction, a lab is promoted/deleted, or a harvested idea ships. |
