...
Prototype branch: `code/sveltekit-prototype`.
...
For SvelteKit prototype work, start a Codex Desktop new worktree from
`code/sveltekit-prototype` and read
`docs/sveltekit-prototype-handoff.md` first.
...
migration-seams work; avoid spending engineering effort on it unless it becomes
a serious product target again.
