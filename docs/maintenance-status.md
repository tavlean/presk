...
Working tree at last update: bulk helpers include a framework-neutral barrel export, strip item selectors, and session summary selectors for future UI/Svelte consumption. Single-image editor work planning is partly extracted into a pure helper. Project identity is documented so the old local folder/archive name is not confused with current Sqush branding. Run `git status --short --branch` for the exact state.
