...
const patchedWebpWrapperOutputDir = join(
  prototypeRoot,
  '.svelte-kit',
  'sqush-generated',
  'codecs',
...
async function patchWebpWrapperFallbackUrl({ sourcePath, assetName }) {
  const source = await readFile(sourcePath, 'utf8');
  const pattern = `var wasmBinaryFile=new URL("${assetName}",import.meta.url).toString()`;
  const replacement = `var wasmBinaryFile="${assetName}"`;

  if (!source.includes(pattern)) {
    throw new Error(
      `Expected WebP wrapper fallback URL pattern for ${assetName}.`,
    );
  }
...
function generatePatchedWebpWrapperShim() {
  return [
    '// This file is autogenerated by prototypes/sveltekit/scripts/sync-sqush-prototype.mjs',
    ...
    "export type { WebPModule } from '../../../../../../../codecs/webp/enc/webp_enc';",
...
await Promise.all([
  ...
  mkdir(patchedWebpWrapperOutputDir, { recursive: true }),
...
await Promise.all([
  ...
  writeFile(
    ...
    await patchWebpWrapperFallbackUrl({
      sourcePath: join(repoRoot, 'codecs', 'webp', 'enc', 'webp_enc.js'),
      assetName: 'webp_enc.wasm',
    }),
  ),
  writeFile(patchedWebpEncoderShimOutputPath, generatePatchedWebpWrapperShim()),
  writeFile(
